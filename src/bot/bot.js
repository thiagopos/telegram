...
import { msgRelatorio, msgNivel } from '../services/reportService.js';
import { readQRCodeFromBuffer } from '../services/qrReader.js';
import { downloadFileBuffer } from '../services/telegram.js';
import { buscarSolicitacao, getAtestadoByCodigo } from '../db/queries.js';
...
bot.on('message', async (ctx) => {
  if (!ctx.processed) {
    if (ctx.message.photo) {
      ...
      return ctx.reply('Por favor, use o menu para solicitar Relatório Geral, Relatório PS ou Validação (envie uma foto com QR Code).');
    }
  }
});
